...
func (l *UpdateGroupJoinTypeLogic) UpdateGroupJoinType(req *types.UpdateGroupJoinTypeReq) (resp *types.UpdateGroupJoinTypeResp, err error) {
	uid := l.custom.UID
	...
	_, err = l.svcCtx.GroupRPC.UpdateGroupJoinType(l.ctx, &groupclient.UpdateGroupJoinTypeReq{
		Gid:      gid,
		Operator: uid,
		Type:     group.GroupJoinType(req.JoinType),
	})
	if err != nil {
		return nil, err
	}

	resp = &types.UpdateGroupJoinTypeResp{}
	return
}
