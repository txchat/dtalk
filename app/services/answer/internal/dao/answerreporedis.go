...
type AnswerRepositoryRedis struct {
	redis *redis.Pool
}

func NewAnswerRepositoryRedis(c xredis.Config) *AnswerRepositoryRedis {
	return &AnswerRepositoryRedis{
		redis: xredis.NewPool(c),
	}
}

func (repo *AnswerRepositoryRedis) AddRecordSeqIndex(uid string, m *model.MsgIndex) error {
...
func (repo *AnswerRepositoryRedis) GetRecordSeqIndex(uid, seq string) (*model.MsgIndex, error) {
	key := keyUserRecordSeq(uid)
	conn := repo.redis.Get()
	defer conn.Close()
